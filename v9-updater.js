(function(){
  ...
  function injectStyle(){
    if($('#k9Style'))return;
    const s=document.createElement('style');s.id='k9Style';
    s.textContent=`
      .splashCenter{animation:k9Enter 1.05s cubic-bezier(.16,1,.3,1) both}
      .splashLogo{animation:k9Float 3.8s cubic-bezier(.4,0,.2,1) infinite}
      .splashCenter:before{animation:k9Glow 3.2s ease-in-out infinite}
      .splashProgress i{animation:k9Load 1.65s cubic-bezier(.4,0,.2,1) infinite}
      .splash:before{animation:k9Orbit 4.8s cubic-bezier(.2,.7,.2,1) infinite}
      .splash:after{animation:k9Orbit 6.4s .8s cubic-bezier(.2,.7,.2,1) infinite reverse}
      @keyframes k9Enter{0%{opacity:0;transform:translateY(24px) scale(.92);filter:blur(7px)}55%{opacity:1;transform:translateY(-4px) scale(1.012);filter:blur(0)}100%{opacity:1;transform:none}}
      @keyframes k9Float{0%,100%{transform:translateY(0)}50%{transform:translateY(-4px)}}
      @keyframes k9Glow{0%,100%{transform:scale(.9);opacity:.4}50%{transform:scale(1.08);opacity:.8}}
      @keyframes k9Orbit{0%{transform:translate(-50%,-50%) scale(.45);opacity:0}18%{opacity:.55}72%{opacity:.12}100%{transform:translate(-50%,-50%) scale(1.35);opacity:0}}
      @keyframes k9Load{0%{transform:translateX(-140%)}55%{transform:translateX(160%)}100%{transform:translateX(360%)}}
      .k9Release{position:fixed;left:14px;right:14px;bottom:18px;z-index:12000;display:flex;gap:12px;align-items:flex-start;padding:15px;border:1px solid var(--line);border-radius:22px;background:color-mix(in srgb,var(--card) 95%,transparent);backdrop-filter:blur(18px);box-shadow:0 18px 55px rgba(0,0,0,.16);animation:k9Sheet .45s cubic-bezier(.16,1,.3,1)}
      ...
      @keyframes k9Sheet{from{opacity:0;transform:translateY(22px) scale(.98)}to{opacity:1;transform:none}}
  ...
  function hideSplash(){
    const el=$('#splash');
    if(el)setTimeout(()=>el.classList.add('hide'),1450);
  }
